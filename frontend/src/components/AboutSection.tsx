        ...
        <div className="grid md:grid-cols-2 gap-10 items-start">
          <motion.div initial={{opacity:0,x:-30}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:0.5,delay:0.1}}>
            <p className="text-lg text-muted-foreground leading-relaxed mb-6 font-sans">
              {settings.about_intro || profile.bio}
            </p>
            <p className="text-muted-foreground leading-relaxed mb-6 font-sans">
              {settings.about_story || `Currently pursuing B.E. CSE at MVSR Engineering College (GPA: ${profile.gpa}).`}
            </p>
            <blockquote className="border-l-4 border-primary pl-4 italic text-foreground/80 font-display text-lg">
              "{settings.about_quote || "Build things that matter. Ship fast. Learn faster."}"
            </blockquote>
          </motion.div>
          <motion.div className="glass-card p-6 space-y-5"
            ...
            <InfoRow icon={GraduationCap} label="Education" value={settings.education?.[0]?.degree ? `${settings.education[0].degree} — ${settings.education[0].institution}` : "B.E. Computer Science — MVSR Engineering College"}/>
